...
public class CacheAspect {
	 ...
	 * 定义缓存逻辑
	 */
	@Around("@annotation(com.sendi.system.redis.Cacheable)")
	public Object cache(ProceedingJoinPoint pjp) {
		...
		Cacheable cacheable = method.getAnnotation(com.sendi.system.redis.Cacheable.class);
		...
		if (result == null) {
			try {
				result = pjp.proceed();
				redis.hset(cacheable.key(), fieldKey, JsonUtil.toJSONString(result));
			} catch (Throwable e) {
				e.printStackTrace();
			}
		}
